"""GlobalBiz 管理画面 v3
新機能: ASIN自動取得 / 利益率設定 / 物流連携 / Shopee多国展開 / 商品一覧改善
"""
...
def get_all_rates() -> dict:
    ...
    return {
        ...
        "TWD": float(get_env("DEFAULT_EXCHANGE_RATE_TWD", "4.7")),
...
if page == "📦 商品一覧":
    ...
    if not products:
        ...
        st.caption(f"{len(products)} 件表示  |  🔴=利益率5%未満  🟡=10%未満")

        # ─── 赤字警告 ───
        low_profit = [p for p in products
                      if (pf := calc_profit_for_product(p)) and pf.get("profit_rate", 1) < 0.05]
        if low_profit:
            with st.expander(f"⚠️ 利益率5%未満の商品 ({len(low_profit)}件)", expanded=True):
                for p in low_profit:
                    pf = calc_profit_for_product(p)
...
# ══════════════════════════════════════════════════════════════════
    ...
    with left:
        ...
        with st.form("product_form", clear_on_submit=True):
            ...
            _asin_images = _pf.get("images", []) or []
            img_rows = []
            for _i in range(0, 9, 3):
                _img_cols = st.columns(3)
                for _j, _col in enumerate(_img_cols):
                    _idx = _i + _j + 1
                    _default_url = _asin_images[_idx-1] if _idx-1 < len(_asin_images) else ""
                    _url = _col.text_input(
                        f"画像 {_idx}", value=_default_url,
                        placeholder="https://...", key=f"img_url_{_idx}",
                    )
...
# ══════════════════════════════════════════════════════════════════
    ...
    tab_ebay, tab_shopee, tab_profit, tab_price, tab_logistics, tab_misc = st.tabs([
    ...
    with tab_price:
        ...
        with st.form("price_form"):
            ...
            twd = r_c3.number_input("TWD レート（台湾）", value=float(get_env("DEFAULT_EXCHANGE_RATE_TWD","4.7")), step=0.1)
